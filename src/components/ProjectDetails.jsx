...
export default function ProjectDetails({ projectState, setProjectState }) {
  ...
  let TasksC = Tasks?.map(task => {
    return (
      <div key={task.tId} className='flex items-center p-2 hover:bg-stone-200 rounded transition cursor-pointer'>
  ...
  return (
    <div className='flex-1 flex justify-center items-center'>
      <div className='bg-stone-100 shadow rounded p-4 w-1/2 max-w-full'>
        <h1 className='text-3xl font-sans flex items-center justify-between'>{pro?.title} <span className='bg-stone-700 flex justify-center items-center rounded-full  text-stone-50 w-16 p-[1.5px] text-sm'>Project</span></h1>
        <p className='bg-stone-200 min-h-44 font-light break-all p-2 mt-6 mb-2 max-w-full'>
          {pro?.desc}
        </p>
        <div className='flex flex-wrap gap-3'>
          <Input label='Add New Task' ref={newTask} />
          <button 
          className='self-end py-1.5 px-8 rounded text-slate-100 bg-stone-700'
          onClick={() => 
